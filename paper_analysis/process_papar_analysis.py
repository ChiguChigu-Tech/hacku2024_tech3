    ...
    if translation_model == 'Google':
        ...
        for word in words:
            try:
                ...
                用語に関する説明は不要です。
                {field_options}の分野に適した意味を教えて下さい。
                {field_options}に関連しない単語は、通常の英語辞書を参照してください。
                """
                ...
                response = client.chat.completions.create(
                    ...
                    messages=[
                        ...
                        {"role": "system", "content": "You do not use the phrase '「', 'の意味は',  '」', ':', or 'です' in your response."},
                ...
                if "の意味は" in translation:
                    # 翻訳を再度取得
                    translation = translation.split("の意味は")[1].strip()
                    response = client.chat.completions.create(
                        model = "gpt-4o-mini",
                        messages=[
                            {"role": "system", "content": "You are a helpful assistant who translates English to Japanese."},
                            {"role": "system", "content": "You do not use the phrase '「', 'の意味は',  '」', ':', or 'です' in your response."},
                            {"role": "user", "content": f"/japanese {re_message}"}
                        ],
                        temperature=0.3,
                        max_tokens=50,
                        n=1
